...
//// main class: "Sum"
  static MainClass.T sun = new MainClassSingle(
		  "Sum", "a", new Print(new Call(
			new NewObject("Doit"), "doit", 
			new util.Flist<Exp.T>().list(new Num(101)))));
  
  // // class "Doit"
  static ast.Ast.Class.T doi = new ast.Ast.Class.ClassSingle("Doit", null,
		  new util.Flist<Dec.T>().list(),
		  //
		  new util.Flist<Method.T>().list(new Method.MethodSingle(
			new Type.Int(), "doit", 
			...
			new util.Flist<Stm.T>().list(new Assign("i", new Num(0)), 
			  new Assign("sum", new Num(0)), new While(
				new Lt(new Id("i"), new Id("n")), new Block(
				  new util.Flist<Stm.T>().list(new Assign("sum", 
				    new Add(new Id("sum"),new Id("i"))))))),
			new Id("sum"))));
		  
  public static Program.T prog = new ProgramSingle(sun, 
		  new util.Flist<ast.Ast.Class.T>().list(doi));
